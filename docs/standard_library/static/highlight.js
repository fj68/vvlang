...
async function initShiki() {
    ...
    preElements.forEach((pre) => {
        ...
        // Support only 'vv' for now, as requested.
        // Check for 'language-vv' class or assume 'vv' if no language class is present.
        const classes = Array.from(codeElement.classList);
        const hasLang = classes.some(c => c.startsWith('language-'));
        const isVV = classes.includes('language-vv');

        if (hasLang && !isVV) return;
